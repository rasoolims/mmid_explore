...
with open(os.path.abspath(sys.argv[3]), "w", encoding="utf-8") as writer:
    ...
    for line in open(os.path.abspath(sys.argv[2]), "r", encoding="utf-8"):
        ...
        if page_url in target_uri_dict:
            texts = target_uri_dict[page_url].decode("utf-8", "ignore").split("\n")
            contains_text = []
            for text in texts:
            ...
            if len(contains_text)>0:
                ...
                image_text_dict[correspond_file_path] = {"word":word.decode("utf-8", "ignore"), "text":contains_text_dict, "url": page_url, "image":image_link}
